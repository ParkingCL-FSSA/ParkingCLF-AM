...
// 2. PIANTONE: CERCA PRENOTAZIONE (Questo sblocca il pulsante Cerca)
app.get('/api/piantone/cerca/:npass', async (req, res) => {
    try {
        const p = req.params.npass.trim().toUpperCase();
        // Cerca una prenotazione che includa la data di oggi
        const r = await pool.query(
...
// 3. PIANTONE: AZIONE ENTRATA/USCITA (Sblocca i pulsanti E/U)
app.post('/api/piantone/azione', async (req, res) => {
    const { id, azione } = req.body; // azione è 'E' o 'U'
    const colonna = azione === 'E' ? 'orario_ingresso' : 'orario_uscita';
    const nuovoStato = azione === 'E' ? 'INGRESSO' : 'USCITO';
    try {
        await pool.query(
            `UPDATE prenotazioni SET stato = $1, ${colonna} = NOW() WHERE id = $2`, 
            [nuovoStato, id]
        );
...
// 4. VEICOLI DENTRO (Aggiornato per riflettere lo stato attuale)
...
// 5. PRENOTAZIONE E PDF (Grafica Originale Salva)
        ...
        doc.on('end', async () => {
            ...
            const html = `
                <div style="font-family: Arial; border: 2px solid #4A90E2; padding: 20px; border-radius: 15px; max-width: 600px;">
                    ...
                    <p>Pass: <b>${p}</b> confirmed from ${formattaDataIT(dInizio)} to ${formattaDataIT(dFine)}</p>
                </div>`;
            await inviaMailBrevoAPI(email, `Pass ${p}`, html, pdfData, `PASS_${p}.pdf`);
            res.json({ success: true });
        });
