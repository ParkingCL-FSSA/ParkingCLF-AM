...
async function cercaPass(passManuale = null, idRecord = null) {
    ...
    try {
        ...
        // RESET BOTTONI STANDARD (con controlli di sicurezza)
        if (btnIngresso) {
        ...
        if (data.trovato) {
            ...
            // 🎯 STATO: DA VERIFICARE
                ...
                if (boxVerifica) {
                    ...
                    // Recupero i riferimenti DOPO l'iniezione HTML
                    const btnPresente = document.getElementById('btn-presente');
                    const btnNonPresente = document.getElementById('btn-non-presente');
                    
                    if (btnPresente) btnPresente.disabled = giaEntrato ? true : false;
                    if (btnNonPresente) btnNonPresente.disabled = false;
            
                    // Assegnazione tramite onclick per evitare listener duplicati
                    if (btnPresente) {
                        btnPresente.onclick = async (ev) => { 
                            ev.preventDefault(); 
                            if (btnPresente.disabled) return;
                    ...
                    if (btnNonPresente) {
                        btnNonPresente.onclick = async (ev) => { 
                            ev.preventDefault(); 
            ...
            // SCADUTO (Verifica Sanatoria)
                ...
                if (!currentPren.orario_ingresso) {
                    ...
                    if (boxVerificaScaduti) {
                        ...
                        const btnScadutoDentro = document.getElementById('btn-scaduto-dentro');
                        const btnScadutoMaiEntrato = document.getElementById('btn-scaduto-mai-entrato');

                        if (btnScadutoDentro) btnScadutoDentro.onclick = (e) => { e.preventDefault(); if (typeof window.eseguiScadutoDentro === 'function') window.eseguiScadutoDentro(); };
                        if (btnScadutoMaiEntrato) btnScadutoMaiEntrato.onclick = (e) => { e.preventDefault(); if (typeof window.eseguiScadutoMaiEntrato === 'function') window.eseguiScadutoMaiEntrato(); };
                    }
                } else {
                    if (btnIngresso) btnIngresso.style.display = 'none';
                    if (btnUscita) {
          ...
          // Se le date oggiStr e dataInizioStr sono in ISO (YYYY-MM-DD), il confronto funziona correttamente.
            ...
            if (currentPren.stato !== 'SCADUTO' && currentPren.stato !== 'MAI_ENTRATO') {
                if (regU) regU.innerHTML = currentPren.orario_uscita
                    ? `<div style="font-size: 14px; font-weight: bold; color: #1e293b; margin-top: 4px; text-align:center;">(U) Registrata il ${fmtData(currentPren.orario_uscita)} ore ${fmtOra(currentPren.orario_uscita)}</div>`
                    : "";
            }
...
async function aggiornaVeicoli() {
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`⚠️ Impossibile recuperare i dati dei veicoli. Stato server: ${res.status}`);
            return;
        }
            ...
            Promise.allSettled(
                ...
                results.forEach((res, index) => {
                    if (res.status === 'rejected' || (res.value && !res.value.ok)) {
                        console.warn(`⚠️ Errore archiviazione DB per pass ${passDaArchiviareSuDB[index].npass}`);
                    }
                });
            });
        }
        ...
        const lista = dati.filter(x => {
            ...
            if (filtroPiantone === 'attivi') {
                ...
                if (x.stato === 'SCADUTO') return false; 
        ...
        .sort((a, b) => {
            if (valeurCercato !== "") {
                const getPriorita = (item) => {
                    ...
                    if (item.stato === 'SCADUTO') return 3;                
        ...
        if (valeurCercato !== "") {
            ...
            if (lista.length > 0) {
                ...
                else if (veicoloTrovato.stato === 'SCADUTO') { label = "⏰ SCADUTO (Trovato da Ricerca)"; colore = "#dc2626"; sfondo = "#fee2e2"; } 
            ...
            if (statoTabella) {
                ...
                statoTabella.classList.remove('badge-blink', 'badge-blink-2');
            }
        } else {
            if (typeof aggiornaGraficaBadge === 'function') {
                aggiornaGraficaBadge();
        ...
        // RENDERING TABELLA HTML
        const containerLista = document.getElementById('lista-veicoli');
        containerLista.innerHTML = lista.map(x => {
            ...
            ">
                ...
                <td style="${baseStyle} ${wDataIng}">${isScadutoEsplicito ? 'NON ENTRATO' : dataIng}</td>
        ...
        // DELEGAZIONE EVENTI CLICK SU TABELLA
        if (!containerLista.dataset.listenerAttached) {
            containerLista.dataset.listenerAttached = "true";
            containerLista.addEventListener('click', async (e) => {
